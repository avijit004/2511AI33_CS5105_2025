// P018.cpp - Target sum (#ways to assign +/- signs to reach k)

#include <bits/stdc++.h>
using namespace std;

// Same reduction as subset-diff: S1 - S2 = target

long long countSubsetsTargetTS(const vector<int>& arr, int target) {
    int n = arr.size();
...
long long brute(const vector<int>& arr, int k) {
    ...
    return countSubsetsTargetTS(arr, target);
}
